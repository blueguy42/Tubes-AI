...
class LocalSearchBot(Bot):
    ...
    def get_utility(self, state: GameState, last_turn: bool) -> int:
        player2Bool = 1
        if last_turn:
            player2Bool = -1
        ...
        for i in range(self.rowLen):
            for j in range(self.colLen):
                rowVal = int(state.board_status[i][j] * player2Bool)
    ...
    def getNeighbor(self, state: GameState, possibleActions: list) -> tuple((GameState, GameAction)):
        ...
        for action in possibleActions:
            ...
            new_player1_turn = state.player1_turn
            ...
            if new_y < (self.NUMBER_OF_DOTS-1) and new_x < (self.NUMBER_OF_DOTS-1):
                ...
                if abs(new_board_status[new_y][new_x]) == 4:
                    pointsScored = True
            ...
            if action.action_type == "row":
                ...
                if new_y >= 1:
                    ...
                    if abs(new_board_status[new_y-1][new_x]) == 4:
                        pointsScored = True
            ...
            if action.action_type == "col":
                ...
                if new_x >= 1:
                    ...
                    if abs(new_board_status[new_y][new_x-1]) == 4:
                        pointsScored = True

            if not pointsScored:
                new_player1_turn = not new_player1_turn

            # hitung nilai utilitas
            this_value = self.get_utility(GameState(new_board_status, new_row_status, new_col_status, new_player1_turn), state.player1_turn)
            if this_value >= value_max:
                value_max = this_value
                chosen_neighbor = GameState(new_board_status, new_row_status, new_col_status, new_player1_turn)
